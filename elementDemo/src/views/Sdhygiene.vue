		...
		<el-dialog title="添加班级信息" :visible.sync="dialogVisible" width="width: 41%">
			<el-form :model="numberValidateForm" ref="numberValidateForm" label-width="100px" class="demo-ruleForm">
				<el-form-item label="班级名称" prop="age">
					<el-input type="text" placeholder="请输入班级名称" v-model="numberValidateForm.age" autocomplete="off">
					</el-input>
				</el-form-item>
			...
			<el-col class="Oinn" :span="24">
				<el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%">
					...
					<el-table-column label="宿舍栋号" width="120">
						<template slot-scope="scope">{{scope.rowbuildingNo}}栋</template>
					</el-table-column>
					<el-table-column label="楼层">
						<template slot-scope="scope">{{scope.storey}}楼</template>
					</el-table-column>
	...
	export default {
		...
		methods: {
			...
			onSubmit() {
				this.axios({
					method: "get",
					url: "http://122.112.253.28:8095/prod-api/smartdor/sdhygiene/list?pageNum=1&pageSize=10&className=" +
			...
			del(index, id) {
				this.axios({
					method: "DELETE",
					url: "http://122.112.253.28:8095/prod-api/smartdor/sdhygiene/deleteByIds/" +
						id,
					headers: {
						Authorization: window.sessionStorage.token
					}
			...
			Xiugai() {
				this.axios({
					method: "PUT",
					url: "http://122.112.253.28:8095/prod-api/smartdor/sdhygiene/update/" +
						this.idd,
					headers: {
						Authorization: window.sessionStorage.token
					},
