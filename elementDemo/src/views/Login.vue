<template>
	<div id="Login">
      ...
      { required: true, message: '用户不能为空'}]">
							<el-input placeholder="用户" prefix-icon="el-icon-user-solid" v-model="ruleForm.user">
							</el-input>
						</el-form-item>
      ...
      { required: true, message: '验证码不能为空'}]">
									<el-input type="text" placeholder="验证码" prefix-icon="el-icon-s-claim"
										v-model="ruleForm.yanz"></el-input>
								</el-form-item>
							</el-col>
					...
					this.$refs[formName].validate(valid => {
						if (valid) {
							if (result.data.code == "200") {
								...
								sessionStorage.setItem("token", result.data.data.tokenHead + " " + result
									.data.data.token);
								this.$notify({
									title: "成功",
									type: "success"
